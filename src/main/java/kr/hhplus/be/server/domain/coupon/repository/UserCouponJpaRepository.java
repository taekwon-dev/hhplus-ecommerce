...
public interface UserCouponJpaRepository extends JpaRepository<UserCoupon, Long> {
    ...
    @Query("SELECT uc.coupon FROM UserCoupon uc WHERE uc.user = :user AND uc.status = :status")
    List<Coupon> findAvailableCouponsByUser(User user, UserCouponStatus status, Pageable pageable);
